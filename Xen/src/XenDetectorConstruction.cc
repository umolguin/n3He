...
G4VPhysicalVolume* XenDetectorConstruction::_GDML()
{
	/// IDs
	    ...
	    density1 = 8.96*g/cm3;
	    a = 63.55*g/mole;
	    G4Material* Cu = new G4Material("Copper"   , 29., a, density1);
	//
	//
	...
	//    G4LogicalVolume *_lCellsDyna[144];
	    ...
	    G4Tubs* _wire= new G4Tubs("wire",0,.025*cm,16.02*cm,0,twopi);
		G4LogicalVolume* _lWire= new G4LogicalVolume(_wire,Cu,"wire");
	//
	...
	//	new G4PVPlacement(0,G4ThreeVector(0,0,(-15.24-(1.25/2))*cm),_cylTar1Log,"Tar1",lv,false,_backWindowID,true);
	    ...
	    for(int i=0; i<9;i++){
	    	new G4PVPlacement(rotm1,G4ThreeVector(0,((8.57-1.905/2)*cm)-(i*_cY),((15.24-1.905/2)*cm)-(-1*_cZ)),_lWire,"Wires",lv,-1,true);
	    	new G4PVPlacement(rotm1,G4ThreeVector(0,((8.57-1.905)*cm)-(i*_cY),((15.24-1.905)*cm)-(-1*_cZ)),_lWire,"Wires",lv,-1,true);
	        for(int j=0;j<16;j++)
	        {

	        	new G4PVPlacement(0, G4ThreeVector(0,((8.57-1.905/2)*cm)-(i*_cY),((15.24-1.905/2)*cm)-(j*_cZ)),_lCells,"CELLS",lv,false,10+(j+(i*16)),true);
	            new G4PVPlacement(rotm1,G4ThreeVector(0,((8.57-1.905)*cm)-(i*_cY),((15.24-1.905)*cm)-(j*_cZ)),_lWire,"Wires",lv,-1,true);
	            new G4PVPlacement(rotm1,G4ThreeVector(0,((8.57-1.905/2)*cm)-(i*_cY),((15.24-1.905/2)*cm)-(j*_cZ)),_lWire,"Wires",lv,-1,true);
...
//	    //_____________GDML
	    ...
	    G4Tubs* _wire= new G4Tubs("wire",0,.025*cm,30*cm,0,twopi);
		G4LogicalVolume* _lWire= new G4LogicalVolume(_wire,Cu,"wire");
